...
- `--max-artifact-bytes`로 큰 artifact를 외부화하고, 시크릿 redaction은 기본 적용되어 stdout/로그의 api key/token/password/bearer/AWS key/GitHub·Slack 토큰/JWT/private key 블록을 [REDACTED]로 치환 (`--no-redact-secrets`로 해제)
...
- OMF meta-skill을 Codex, Claude Code, Hermes, generic runtime용 resource로 설치
- 기본 동작은 non-destructive이며 기존 `AGENTS.md`/`CLAUDE.md`가 있으면 patch plan을 생성하고 자동 overwrite하지 않음
- `--dry-run`으로 쓰기 전 계획을 확인하고, 명시적으로 `--overwrite`를 줄 때만 대상 파일을 교체
- Hermes/generic처럼 profile layout이 runtime마다 다른 경우 `--out` 아래 install resource와 profile patch를 생성

## /install mcp

- `omf install mcp --client generic --out .omf/mcp.json`은 OMF stdio MCP 서버 설정을 생성
- 생성된 설정은 `omf mcp serve`를 실행해 다음 tool을 노출:
  `omf_start_session`, `omf_record_input`, `omf_record_artifact`,
  `omf_record_validation`, `omf_record_decision`, `omf_record_event`,
  `omf_finish_session`, `omf_materialize_session`, `omf_promote_capability`,
  `omf_export_capability`, `omf_health`
- MCP tool은 CLI subprocess wrapper가 아니라 application workflow를 직접 호출

## /session

- active agent 작업을 lightweight session으로 추적하고, 완료 후 immutable EvidenceRecord로 materialize
- `session start`는 runtime/model/project/goal/activation_source를 저장
- `session event`는 goal, assumption, context, command, diff, test_result, artifact, user_feedback, decision, completion event를 append
- `session finish`는 success/failure/unknown outcome을 기록하고, `session materialize`는 `.omf/evidence`에 evidence를 생성
- `session suggest-capability`는 goal에서 capability 후보 이름을 제안해 promote 전 이름 선택을 돕는다
...
- Track: `session start`, `session event`, `session finish`, `session materialize`, `session suggest-capability`
- MCP: `mcp serve`
